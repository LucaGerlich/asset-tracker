...
interface ComponentCategoriesTableProps {
...
export default function ComponentCategoriesTable({
  ...
  const renderCell = (item: ComponentCategory, columnKey: string) => {
    switch (columnKey) {
      case "name":
        return item.name;
      case "actions":
        return (
          <div className="flex justify-end gap-1">
            <Link href={`/componentCategories/${item.id}/edit`}>
    ...
    <div className="w-full space-y-4">
      <div className="flex flex-col gap-4">
        <div className="flex items-center justify-between">
          <h1 className="text-2xl font-semibold">Component Categories</h1>
        </div>
        <div className="flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between">
          ...
          <div className="flex flex-wrap gap-3">
            <Button asChild>
              <Link href="/componentCategories/create">
        ...
        <div className="flex flex-wrap items-center justify-between gap-3">
          <span className="text-muted-foreground text-sm">
            Showing {paginatedItems.length} of {filteredItems.length} component
            categories
          </span>
        </div>
      </div>
      <ResponsiveTable
        ...
        keyExtractor={(item: ComponentCategory) => item.id}
        emptyMessage="No component categories found"
        mobileCardView={true}
      />
