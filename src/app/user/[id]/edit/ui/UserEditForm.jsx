      ...
      <form onSubmit={onSubmit} className="flex flex-col gap-4 sm:gap-6">
        <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
          ...
          <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2">
            <Button type="button" variant="light" onPress={() => { if (!isDirty || confirm("Discard unsaved changes?")) router.back(); }} className="w-full sm:w-auto">Cancel</Button>
            <Button color="primary" type="submit" isLoading={saving} className="w-full sm:w-auto">Save</Button>
          </div>
        </div>
        <Separator />
          ...
          <section className="col-span-1 rounded-lg border border-default-200 p-4">
            ...
            <div className="grid grid-cols-1 gap-3">
              ...
              <div>
                ...
                />
                {usernameTaken && <p className="text-sm text-red-500 mt-1">Username already exists</p>}
              </div>
              <div>
          ...
          <section className="col-span-1 rounded-lg border border-default-200 p-4">
            ...
            <div className="grid grid-cols-1 gap-3">
              <div>
                ...
                />
                {emailTaken && <p className="text-sm text-red-500 mt-1">Email already exists</p>}
              </div>
            </div>
          </section>
          ...
          <section className="col-span-1 rounded-lg border border-default-200 p-4">
            ...
            <div className="flex flex-col gap-3">
              <div className="flex gap-2">
                <Button size="sm" variant={!form.isadmin && !form.canrequest ? "default" : "outline"} onClick={() => setForm((f) => ({ ...f, isadmin: false, canrequest: false }))}>Deactivated</Button>
                <Button size="sm" variant={!form.isadmin && form.canrequest ? "default" : "outline"} onClick={() => setForm((f) => ({ ...f, isadmin: false, canrequest: true }))}>Requester</Button>
                <Button size="sm" variant={form.isadmin ? "default" : "outline"} onClick={() => setForm((f) => ({ ...f, isadmin: true, canrequest: true }))}>Admin</Button>
              </div>
        ...
        <div className="flex flex-col sm:flex-row justify-end gap-2">
          ...
          <Button color="primary" type="submit" isLoading={saving} isDisabled={usernameTaken || emailTaken} className="w-full sm:w-auto">Save</Button>
        </div>
      </form>
    </div>
  );
